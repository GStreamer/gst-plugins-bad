...
static void			gst_glsink_chain		(GstPad *pad, GstBuffer *buf);
...
gst_glsink_init (GstGLSink *sink)
{
  ...
  GST_DEBUG (0, "glsink: creating bufferpool");
...
gst_glsink_sinkconnect (GstPad *pad, GstCaps *caps)
{
  ...
  GST_DEBUG (0, "glsink: setting %08x (%4.4s) %dx%d\n", 
...
static void
gst_glsink_chain (GstPad *pad, GstBuffer *buf)
{
  GstGLSink *sink;
  ...
  GST_DEBUG (0,"glsink: clock wait: %llu %u", 
