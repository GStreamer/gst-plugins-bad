...
gst_amf_rate_control_get_type (void)
{
  ...
  if (!rate_control_type) {
    rate_control_type =
        g_enum_register_static ("GstAMFRateControlH264", rate_control);
  }
...
gst_amf_usage_get_type (void)
{
  ...
  if (!usage_type) {
    usage_type = g_enum_register_static ("GstAMFUsageH264", usage);
  }
  return usage_type;
}
...
gst_amf_quality_preset_get_type (void)
{
  ...
  if (!quality_preset_type) {
    quality_preset_type =
        g_enum_register_static ("GstAMFQualityPresetH264", quality_preset);
  }
...
gst_amf_profile_get_type (void)
{
  ...
  if (!profile_type) {
    profile_type = g_enum_register_static ("GstAMFProfileH264", profile);
  }
  return profile_type;
}
...
gst_amf_preencode_get_type (void)
{
  ...
  if (!preencode_type) {
    preencode_type = g_enum_register_static ("GstAMFPreencodeH264", preencode);
  }
...
gst_amf_coding_type_get_type (void)
{
  ...
  if (!coding_type_type) {
    coding_type_type =
        g_enum_register_static ("GstAMFCodingTypeH264", coding_type);
  }
...
static gboolean
setup_encoder (GstAMFh264Enc * enc)
{
  ...
  SET_AMF_VALUE_OR_FAIL (base_enc->encoder_amf,
      AMF_VIDEO_ENCODER_QUALITY_PRESET, enc->quality_preset);
  ...
  if (GetRuntimeVersion (base_enc->amf_ctx) <
    ...
    if (res != AMF_OK) {
      AMF_LOG_WARNING ("Failed to set encoder color range, error code %d.",
          result);
    }
  } else {
  ...
  SET_AMF_VALUE (base_enc->encoder_amf, AMF_VIDEO_ENCODER_ENABLE_VBAQ, true);
  ...
  if (enc->rate_control != AMF_VIDEO_ENCODER_RATE_CONTROL_METHOD_CONSTANT_QP) {
    SET_AMF_VALUE_OR_FAIL (base_enc->encoder_amf,
        AMF_VIDEO_ENCODER_TARGET_BITRATE, bitrate);

    SET_AMF_VALUE_OR_FAIL (base_enc->encoder_amf,
        AMF_VIDEO_ENCODER_PEAK_BITRATE, bitratePeak);
  }
  SET_AMF_VALUE_OR_FAIL (base_enc->encoder_amf, AMF_VIDEO_ENCODER_ENFORCE_HRD,
      enc->enforce_hdr);

  SET_AMF_VALUE (base_enc->encoder_amf,
      AMF_VIDEO_ENCODER_HIGH_MOTION_QUALITY_BOOST_ENABLE, enc->motion_boost);
...
static gboolean
init_h264_encoder (GstVideoEncoder * encoder, GstVideoCodecFrame * frame)
{
      ...
      for (int i = 0; i < gst_buffer_n_memory (frame->input_buffer); i++) {
        ...
        if (!enc->initialised) {
          ...
          res =
              ...
              AMFVideoEncoderVCE_AVC, &enc->encoder_amf);
          if (res != AMF_OK) {
...
static gboolean
is_sync_point_h264 (const amf::AMFBufferPtr & packetData)
{
  uint64_t pktType;
