...
gst_amf_rate_control_get_type (void)
{
  ...
  if (!rate_control_type) {
    rate_control_type =
        g_enum_register_static ("GstAMFRateControlHEVC", rate_control);
  }
...
gst_amf_usage_get_type (void)
{
  ...
  if (!usage_type) {
    usage_type = g_enum_register_static ("GstAMFUsageHEVC", usage);
  }
  return usage_type;
}
...
gst_amf_quality_preset_get_type (void)
{
  ...
  if (!quality_preset_type) {
    quality_preset_type =
        g_enum_register_static ("GstAMFQualityPresetHEVC", quality_preset);
  }
...
gst_amf_profile_get_type (void)
{
  ...
  if (!profile_type) {
    profile_type = g_enum_register_static ("GstAMFProfileHEVC", profile);
  }
  return profile_type;
}
...
gst_amfh265enc_class_init (GstAMFh265EncClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_PREENCODE,
      g_param_spec_boolean ("enable-pre-encode",
          "Pre-encode assisted rate control ",
          "Enables pre-encode assisted rate control ", FALSE,
...
static gboolean
setup_encoder (GstAMFh265Enc * enc)
{
  ...
  amf_double frameRateFraction = ((amf_double) fps_d / (amf_double) fps_n);
  ...
  SET_AMF_VALUE_OR_FAIL (base_enc->encoder_amf,
      AMF_VIDEO_ENCODER_HEVC_FRAMESIZE, AMFConstructSize (base_enc->frameW,
          base_enc->frameH));
  ...
  SET_AMF_VALUE_OR_FAIL (base_enc->encoder_amf,
      AMF_VIDEO_ENCODER_HEVC_QUALITY_PRESET, enc->quality_preset);

  SET_AMF_VALUE_OR_FAIL (base_enc->encoder_amf,
      AMF_VIDEO_ENCODER_HEVC_PROFILE, enc->profile);
  ...
  SET_AMF_VALUE (base_enc->encoder_amf,
      AMF_VIDEO_ENCODER_HEVC_PRE_ANALYSIS_ENABLE, enc->preencode_mode);
  ...
  if (enc->rate_control != AMF_VIDEO_ENCODER_RATE_CONTROL_METHOD_CONSTANT_QP) {
    SET_AMF_VALUE_OR_FAIL (base_enc->encoder_amf,
        AMF_VIDEO_ENCODER_HEVC_TARGET_BITRATE, bitrate);

    SET_AMF_VALUE_OR_FAIL (base_enc->encoder_amf,
        AMF_VIDEO_ENCODER_HEVC_PEAK_BITRATE, bitratePeak);
  }
  SET_AMF_VALUE_OR_FAIL (base_enc->encoder_amf,
      AMF_VIDEO_ENCODER_HEVC_ENFORCE_HRD, enc->enforce_hdr);
  ...
  SET_AMF_VALUE_OR_FAIL (base_enc->encoder_amf,
      ...
      static_cast < amf_int64 > (bitrate * enc->buffer_size));
  ...
  GetTrace (base_enc->amf_ctx)->SetGlobalLevel (AMF_TRACE_ERROR);
...
static gboolean
init_h265_encoder (GstVideoEncoder * encoder, GstVideoCodecFrame * frame)
{
  ...
  switch (enc->mem_type) {
    ...
    case GST_AMF_MEM_TYPE_D3D11:
    {
      ...
      for (int i = 0; i < gst_buffer_n_memory (frame->input_buffer); i++) {
        GstD3D11Memory *mem =
            (GstD3D11Memory *) gst_buffer_peek_memory (frame->input_buffer, 0);

        if (!enc->initialised) {
          AMF_RESULT res = AMF_FAIL;
          ID3D11Device *handle =
              gst_d3d11_device_get_device_handle (mem->device);
          ...
          res =
              GetFactory (enc->amf_ctx)->CreateComponent (enc->context,
              AMFVideoEncoder_HEVC, &enc->encoder_amf);
          if (res != AMF_OK) {
...
static gboolean
is_sync_point_h265 (const amf::AMFBufferPtr & packetData)
{
  uint64_t pktType;
